...
import src.util.dataset.pam as pam_utils
import boto3
...
class PamSubDataset(BaseDataSubset):
    ...
    def __init__(self, data: List[pam_utils.PAMData]) -> None:
        """
        Initializes the dataset with the provided PAM data.
...
class PAMSubMock(BaseDataSubset):
    """
    A dataset class for handling PAM data subsets.
    ...
    def __init__(self, data: List[pam_utils.PAMData], divisor: int = 20) -> None:
        """
        Initializes the dataset with the provided PAM data.
        ...
        super().__init__(data)
        import logging
        logging.warning("!"*64+"\nWARNING\t\tUSING THE MOCK DATASET (PAM)\t\tWARNING\n" + "!"*64)
...
class PAMDataset(BaseDataset):
    ...
    Attributes:
        __pam_url (str): URL to download the PAM dataset.
        __pam_raw_zip_filename (str): Filename for the raw PAM dataset zip file.
    ...
    __pam_url = "https://figshare.com/ndownloader/files/34683103"
    __pam_raw_zip_filename = "pam_raw.zip"
    __raw_file_names = {
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        """
        self._dataset_class = PamSubDataset # PAMSubMock or PAMSubDataset %TODO: Delete Submock
    ...
    def __getitem__(self, index) -> pam_utils.PAMData:
    ...
    def download(self, **kwargs):
        ...
        if self.aws_profile is not None or self.s3_bucket_path is not None: # load data from aws bucket
            ...
            if not osp.exists(osp.join(self.dataset_dir_paths['raw'], self.__pam_raw_zip_filename)):
                # Downloading data 
                self.cli_logger.info(f'Downloading raw {DatasetOptions.pam} dataset from {self.__pam_url}!')
                pam_utils.download_pam(
                    url=self.__pam_url,
                    folder_path=self.dataset_dir_paths['raw'],
                    file_name=self.__pam_raw_zip_filename,
    ...
    def process_centralized(self, instance: str, **kwargs):
        ...
        self._train_data = [pam_utils.PAMData(data=x, label=y) for x, y in zip(X_train, y_train)]
        self._val_data = [pam_utils.PAMData(data=x, label=y) for x, y in zip(X_val, y_val)]
        self._test_data = [pam_utils.PAMData(data=x, label=y) for x, y in zip(X_test, y_test)]
