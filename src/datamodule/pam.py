...
from src.util import DatasetOptions
...
def collate_fn(batch):
    ...
    return pam_utils.PAMBatchData(data=data, label=label, seq_len=seq_len)

def collate_fn_padded_seq(batch: List[pam_utils.PAMData]):
    """
    ...
    label = torch.concat(label)
    seq_len = torch.concat(seq_len)
    return pam_utils.PAMBatchData(data=data_padded, label=label, seq_len=seq_len)
